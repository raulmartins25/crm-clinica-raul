...
import {
  ...
  Bot, BotOff, Image, FileText, Mic, X, Check, CheckCheck,
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center justify-between mt-0.5">
                    <p className="text-xs text-gray-500 truncate">
                      {truncate(conv.lastMessageText || '', 38)}
                    </p>
                    {conv.unreadCount > 0 && (
      ...
      {selectedConv ? (
        <div className="flex-1 flex flex-col min-w-0">
          ...
          <div className="flex items-center gap-3 px-5 py-3.5 bg-white border-b border-gray-100 shadow-sm">
            ...
            <div className="flex-1 min-w-0">
              <p className="font-semibold text-gray-900">{displayName(selectedConv)}</p>
...
function MessageBubble({ message, currentUserId }: { message: Message; currentUserId: string }) {
  ...
  const statusIcon = () => {
    ...
    switch (message.status) {
      case 'READ': return <CheckCheck className="w-3.5 h-3.5 text-sky-400" />
      case 'DELIVERED': return <CheckCheck className="w-3.5 h-3.5 text-gray-400" />
      default: return <Check className="w-3.5 h-3.5 text-gray-400" />
    }
  }
        ...
        return (
          <div>
            {message.mediaUrl ? (
              ...
              <img src={message.mediaUrl} alt="imagem" className="rounded-lg max-w-xs max-h-48 object-cover" />
            ) : (
  ...
  return (
    <div className={cn('flex animate-fade-in', isOut ? 'justify-end' : 'justify-start')}>
      <div className={cn('max-w-[75%]', isOut ? 'items-end' : 'items-start', 'flex flex-col')}>
        {message.isFromAI && (
          <span className="text-xs text-green-500 mb-1 flex items-center gap-1">
            <Bot className="w-3 h-3" /> IA
          </span>
        )}
        <div className={cn(isOut ? 'chat-bubble-out' : 'chat-bubble-in')}>
          {renderContent()}
